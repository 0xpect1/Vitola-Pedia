// expand-catalog-v4.cjs — Generate 5,000 MORE new cigars (second batch)
// Target: 25,815 total cigars (currently 20,815)
// Strategy:
//   1. Focus on brands that still have <40 cigars — generate more vitola/wrapper/blend variants
//   2. Add new blend lines not yet used: Reserve, Maduro Edition, Habano Edition, Vintage,
//      Signature, Heritage, Classic, Limited Reserve, Anniversary Edition, Master Series
//   3. Generate new vitola sizes not yet covered for each brand
//   4. Create realistic prices, ratings, smoking times, strengths, flavor profiles
//   5. Generate unique IDs (brand-slug-vitola-wrapper-suffix if needed for uniqueness)
...
// Vitolas with length/ring gauge — expanded with new sizes not in v3
const VITOLAS = [
  ...
  // New vitola sizes for v4
  { size: 'Petit Corona', length: 4.5, ringGauge: 42, smokeTime: 30 },
  { size: 'Short Robusto', length: 4.5, ringGauge: 50, smokeTime: 40 },
  { size: 'Extra Toro', length: 6.5, ringGauge: 54, smokeTime: 70 },
  { size: 'Grand Churchill', length: 7.5, ringGauge: 50, smokeTime: 100 },
  { size: 'Slim Panatela', length: 7.0, ringGauge: 38, smokeTime: 70 },
  { size: 'Culebra', length: 6.0, ringGauge: 39, smokeTime: 60 },
  { size: 'Robusto Extra', length: 5.5, ringGauge: 52, smokeTime: 60 },
  { size: 'Double Corona', length: 7.5, ringGauge: 49, smokeTime: 100 },
  { size: 'Presidente', length: 8.0, ringGauge: 52, smokeTime: 110 },
  { size: 'Super Gordo', length: 6.0, ringGauge: 64, smokeTime: 80 },
  { size: 'Nub', length: 4.0, ringGauge: 60, smokeTime: 40 },
  { size: 'Short Panatela', length: 5.0, ringGauge: 36, smokeTime: 35 },
  { size: 'Gordito', length: 5.0, ringGauge: 56, smokeTime: 50 },
  { size: 'Rothchild', length: 4.5, ringGauge: 50, smokeTime: 40 },
  { size: 'Demihagre', length: 5.5, ringGauge: 40, smokeTime: 45 },
  { size: 'Long Robusto', length: 5.5, ringGauge: 50, smokeTime: 55 },
  { size: 'Box-Pressed Robusto', length: 5.0, ringGauge: 52, smokeTime: 55 },
  { size: 'Box-Pressed Gordo', length: 6.0, ringGauge: 60, smokeTime: 70 },
  { size: 'Salomon', length: 6.5, ringGauge: 54, smokeTime: 75 },
  { size: 'Diadema', length: 8.0, ringGauge: 55, smokeTime: 120 },
];
...
// NEW blend lines for v4 — distinct from v3's set
const BLEND_LINES = [
  { suffix: 'Reserve', limited: false, priceMod: 1.3, ratingMod: 1 },
  ...
  { suffix: 'Limited Reserve', limited: true, priceMod: 1.7, ratingMod: 2 },
  { suffix: 'Anniversary Edition', limited: false, priceMod: 1.5, ratingMod: 2 },
  { suffix: 'Master Series', limited: false, priceMod: 1.6, ratingMod: 2 },
];
...
// Deterministic pseudo-random for reproducibility — different seed from v3
let seed = 9182736;
function rand() {
...
// Track which (brand, blendLine, wrapper, vitola) combos we've already used
// to avoid duplicates within this batch
const usedCombos = new Set();

for (const brand of brandList) {
  if (generated >= TARGET) break;

  // Focus on brands with <40 cigars — generate more for them
  // Brands with <40 get up to 30 new cigars; brands with 40-80 get up to 15; larger brands get 8
  const maxForBrand = brand.count < 40 ? 30 : brand.count < 80 ? 15 : 8;
  ...
  for (const blendLine of BLEND_LINES) {
    ...
    for (const wrapper of WRAPPERS) {
      ...
      for (const vitola of VITOLAS) {
        ...
        const comboKey = `${brand.brand}|${blendLine.suffix}|${wrapper.wrapper}|${vitola.size}`;
        if (usedCombos.has(comboKey)) { skipped++; continue; }
        ...
        // Make ID unique by adding a suffix if needed
        let finalId = id;
        let counter = 1;
