/**
 * Retry scraping Havana House images for Cuban cigars that were missed.
 * Reads:  /tmp/hh_retry.json        [{id, name, url}]
 * Output: /tmp/hh_retry_images.json  [{id, image}]
 */
...
async function main() {
  console.log(`Retrying HH images for ${DATA.length} Cuban cigars...`);
  ...
  writeFileSync('/tmp/hh_retry_images.json', JSON.stringify(results, null, 2));
  console.log(`\n✓ Done! Found ${results.length} images → /tmp/hh_retry_images.json`);
